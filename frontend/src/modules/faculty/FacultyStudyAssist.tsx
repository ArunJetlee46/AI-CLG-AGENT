...
import { facultyApi, type FacultyStudyAnswer } from "@/modules/faculty/api";
...
interface Message {
  role: "user" | "assistant";
  content: string;
  sources?: FacultyStudyAnswer["sources"];
  grounded?: boolean;
  meta?: string;
  error?: boolean;
}
...
const SUGGESTIONS = [
  "What is covered in the OS syllabus?",
  "Explain database normalization forms",
  "Summarize compiler design phases",
  "What are the prerequisites for algorithms?",
];

export function FacultyStudyAssist() {
  const token = useAuthStore((s) => s.token);
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hello! I'm the curriculum study assistant for faculty. Ask any question about your courses and syllabus — answers are grounded in the university curriculum knowledge base.",
    },
  ]);
  ...
  async function send(event: FormEvent, preset?: string) {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "assistant", content: "", meta: "thinking…" },
    ]);
    setInput("");
    setBusy(true);

    try {
      const result = await facultyApi.askStudyAssistant(token, question);
      setMessages((prev) => {
        const copy = [...prev];
        ...
        if (last.role === "assistant") {
          copy[copy.length - 1] = {
            ...
            meta: `sources ${result.sources.length} · ${result.grounded ? "grounded" : "unavailable"}`,
          };
        }
        return copy;
      });
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        ...
        subtitle="Curriculum-grounded Q&A for your courses"
        icon={MessagesSquare}
      ...
      <Card className="flex h-[calc(100vh-12rem)] min-h-[28rem] flex-col overflow-hidden">
        <div ref={scrollRef} className="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
          {messages.map((message, index) => (
            ...
            >
              {message.role === "assistant" && (
                <span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-teal-600 text-white">
              ...
              >
                ...
                {message.sources && message.sources.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5 border-t border-[var(--border)]/60 pt-2">
          ...
          {busy && (
            <div className="fade-up flex items-center gap-2.5 self-start">
              <span className="grid h-7 w-7 place-items-center rounded-full bg-teal-600 text-white">
                <MessagesSquare className="h-4 w-4" />
              </span>
        ...
        >
          <Input
            ...
            placeholder='Try: "What topics are in the OS syllabus?"'
            disabled={busy}
